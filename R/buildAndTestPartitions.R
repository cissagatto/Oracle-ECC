...
partition <- function(id_part, ds, dataset_name, number_folds, namesLabels, folderResults){
  ...
  FolderPartition = paste(diretorios$folderResultDataset,
                          "/Partition-", info$numberOfPartition, sep="")
  ...
  while(f<=10){
    ...
    while(g<=info$numberGroupsOfPartition){
      ...
      e = as.numeric(length(specificGroup$labels))
      ...
      if(e<=1){
        ...
        # treino
        ds_train = mldr_from_dataframe(grupo_tr,
                                       labelIndices = indices)

        # teste
        ds_test = mldr_from_dataframe(grupo_ts,
                                      labelIndices = indices)
        ...
        # Predict
        test_ecc <- predict(eccmodel, ds_test, cores = number_cores)

        # Apply a threshold
        thresholds_ecc <- scut_threshold(test_ecc,
                                         ds_test,
                                         cores = number_cores)
        new.test.ecc <- fixed_threshold(test_ecc, thresholds_ecc)

        setwd(FolderGroup)
        write.csv(new.test.ecc, "y_predict.csv", row.names = FALSE)
